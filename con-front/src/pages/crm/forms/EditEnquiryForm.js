...
const EditEnquiryForm = ({ isOpen, onClose, onSubmit, enquiry }) => {
  ...
  // Load enquiry data when component mounts or enquiry changes
  useEffect(() => {
    if (enquiry) {
      setFormData({
        contact_person_name: enquiry.contact_person_name || '',
        contact_surname: enquiry.contact_surname || '',
        ...
        city: enquiry.city || '',
        state: enquiry.state || '',
        project_type: enquiry.project_type || 'Residential',
        construction_type: enquiry.construction_type || 'New Construction',
        building_type: enquiry.building_type || '',
        approximate_area: enquiry.approximate_area || '',
        area_unit: enquiry.area_unit || 'sqft',
        budget_range: enquiry.budget_range || '',
        expected_timeline: enquiry.expected_timeline || '',
        ...
        landing_page: enquiry.landing_page || '',
        device_type: enquiry.device_type || 'Desktop',
        enquiry_notes: enquiry.enquiry_notes || '',
        has_specific_location: enquiry.has_specific_location || false,
        has_realistic_budget: enquiry.has_realistic_budget || false,
        has_immediate_timeline: enquiry.has_immediate_timeline || false,
        is_repeat_visitor: enquiry.is_repeat_visitor || false,
        status: enquiry.status || 'New'
      });

      // Set selected package details if package_id exists
      if (enquiry.package_id && packages.length > 0) {
        const selectedPackage = packages.find(pkg => pkg.id == enquiry.package_id);
        if (selectedPackage) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const updatedData = {
        ...
        // Update classification based on changes
      ...
      const response = await enquiriesAPI.update(enquiry.enquiry_id, updatedData);
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-5xl w-full max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b">
          <div>
            ...
            <p className="text-gray-600">Update enquiry details - {enquiry.enquiry_number}</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6">
            ...
            <div className="space-y-6">
              ...
              <div>
                <div className="flex items-center space-x-2 mb-4">
                  ...
                  <h4 className="text-lg font-semibold text-gray-900">Status & Additional Info</h4>
                </div>
              ...
              {/* Qualification Flags */}
              <div>
                <h4 className="text-md font-semibold text-gray-900 mb-3">Lead Qualification</h4>
          ...
          <div className="flex items-center justify-end space-x-4 mt-8 pt-6 border-t">
            ...
            >
              ...
              <span>{isSubmitting ? 'Updating...' : 'Update Enquiry'}</span>
            </button>
          </div>
        </form>
      </div>
