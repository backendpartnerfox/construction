...
import { Search, Plus, Edit, Trash2, X, Save, Wallet } from 'lucide-react';
...
const PaymentTypesManagement = () => {
  ...
  const fetchTypes = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${API_BASE_URL}/api/payment_types`);
  ...
  const handleAdd = () => {
    setEditingType(null);
    setFormData({ payment_type_name: '', payment_category: '', is_active: true });
    setShowModal(true);
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/payment_types/${id}`, { method: 'DELETE' });
      if (response.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.payment_type_name.trim()) {
      alert('Please enter payment type name');
      return;
    }
    try {
      const url = editingType
        ? `${API_BASE_URL}/api/payment_types/${editingType.payment_type_id}`
        : `${API_BASE_URL}/api/payment_types`;
      ...
      if (response.ok) {
        alert(`Payment type ${editingType ? 'updated' : 'created'} successfully`);
        setShowModal(false);
        fetchTypes();
      } else {
        const error = await response.json();
  ...
  const filteredTypes = types.filter(type =>
    type.payment_type_name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    type.payment_category?.toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    <div className="p-6">
      <div className="mb-6">
        <div className="flex items-center gap-2 mb-2">
          ...
          <h1 className="text-2xl font-bold text-gray-800">Payment Types Management</h1>
        </div>
        <p className="text-gray-600">Manage payment type master data</p>
      </div>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {loading ? (
              ...
              <tr><td colSpan="5" className="px-6 py-4 text-center text-gray-500">No payment types found</td></tr>
            ) : (
              filteredTypes.map((type) => (
                <tr key={type.payment_type_id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{type.payment_type_id}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{type.payment_type_name}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{type.payment_category || '-'}</td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    ...
                    <button onClick={() => handleDelete(type.payment_type_id)} className="text-red-600 hover:text-red-900">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold">{editingType ? 'Edit Payment Type' : 'Add New Payment Type'}</h2>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    required
                    placeholder="e.g., Advance, Balance, Milestone"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    placeholder="e.g., Project, Vendor"
                  />
                </div>
