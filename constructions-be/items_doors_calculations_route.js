 ...
 *                 type: object
 */
router.get('/items-doors-calculations', async (req, res) => {
  const db = req.db;
  ...
  try {
    let query = `
      SELECT 
        ...
        p.project_code,
        e.element_name,
        dd.description as dimension_description,
      ...
      LEFT JOIN door_dimensions dd ON idc.dimension_id = dd.dimension_id
 ...
 */
router.get('/items-doors-calculations/:id', async (req, res) => {
  const db = req.db;
  const { id } = req.params;
  ...
  try {
    const result = await db.query(`
      SELECT 
        ...
        e.element_name,
        dd.description as dimension_description,
        dd.width as standard_width,
        dd.height as standard_height,
        dd.thickness as standard_thickness,
        emp.first_name || ' ' || emp.last_name AS calculated_by_name,
        emp2.first_name || ' ' || emp2.last_name AS verified_by_name
      FROM items_doors_calculations idc
      LEFT JOIN projects p ON idc.project_id = p.project_id
 ...
 *         description: Internal server error
 */
router.post('/items-doors-calculations', async (req, res) => {
  const db = req.db;
  const {
    project_id,
    element_id,
  ...
  const requiredFields = {
    ...
    dimension_id, door_material, door_style
  };
  ...
  try {
    const result = await db.query(
      `INSERT INTO items_doors_calculations (
        ...
        door_finish, dimension_id, quantity, custom_design,
      ...
      [
        project_id, element_id, measurement_id, floor, room,
        location_description, wall_direction, door_material, door_style,
        door_finish, dimension_id, quantity || 1, custom_design || false,
        custom_design_description, custom_image_path, polish_type,
 ...
 *         description: Internal server error
 */
router.put('/items-doors-calculations/:id', async (req, res) => {
  const db = req.db;
  const { id } = req.params;
 ...
 *         description: Internal server error
 */
router.delete('/items-doors-calculations/:id', async (req, res) => {
  const db = req.db;
  const { id } = req.params;
...
/**
 * @swagger
 * /items-doors-calculations/{id}/verify:
 *   post:
 ...
 *         description: Internal server error
 */
router.post('/items-doors-calculations/:id/verify', async (req, res) => {
  const db = req.db;
  const { id } = req.params;
...
/**
 * @swagger
 * /items-doors-calculations/project/{projectId}/summary:
 *   get:
 ...
 *         description: Internal server error
 */
router.get('/items-doors-calculations/project/:projectId/summary', async (req, res) => {
  const db = req.db;
...
/**
 * @swagger
 * /items-doors-calculations/project/{projectId}/floor/{floor}:
 *   get:
 ...
 *         description: Internal server error
 */
router.get('/items-doors-calculations/project/:projectId/floor/:floor', async (req, res) => {
  const db = req.db;
...
/**
 * @swagger
 * /items-doors-calculations/project/{projectId}/hardware-summary:
 *   get:
 ...
 *         description: Internal server error
 */
router.get('/items-doors-calculations/project/:projectId/hardware-summary', async (req, res) => {
  const db = req.db;
  const { projectId } = req.params;
...
/**
 * @swagger
 * /items-doors-calculations/project/{projectId}/special-features:
 *   get:
 ...
 *         description: Internal server error
 */
router.get('/items-doors-calculations/project/:projectId/special-features', async (req, res) => {
  const db = req.db;
  const { projectId } = req.params;
...
/**
 * @swagger
 * /items-doors-calculations/door-dimensions:
 *   get:
 ...
 *         description: Internal server error
 */
router.get('/items-doors-calculations/door-dimensions', async (req, res) => {
  const db = req.db;
