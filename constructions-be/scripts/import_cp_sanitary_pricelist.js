/*
 * Import CP & Sanitary section of MEP_Procurement_Master_Pricelist.xlsx.
 * Creates: 12 items, 120 item_choices, 120 item_choice_pricing.
 * NO package_items_mapping — sheet has no package matrix.
 *
 * Usage:  node scripts/import_cp_sanitary_pricelist.js
 */
...
const SOURCE_TAG = '[mep-cp-sanitary-import-v1]';
const CATEGORY = 'CP & Sanitary';
...
async function main() {
  ...
  const rows = XLSX.utils.sheet_to_json(wb.Sheets['CP & Sanitary Pricelist Matrix'], { defval: '' });
  ...
  try {
    ...
    for (const r of rows) {
      const name = r['CP & Sanitary Item List'].toString().trim();
    ...
    for (const [name, unit] of itemUnitByName) {
      const r = await client.query(
        `INSERT INTO items (item_name, item_description, item_unit, item_category, is_active)
         VALUES ($1, $2, $3, $4, TRUE)
         RETURNING item_id`,
        [name, `${SOURCE_TAG} ${name}`, unit, CATEGORY]
      );
    ...
    for (const r of rows) {
      ...
      const series = r['Series Line'].toString().trim();
      const itemName = r['CP & Sanitary Item List'].toString().trim();
      ...
      const choiceRes = await client.query(
        ...
        [itemId, 'CP & Sanitary', brand, series, displayName, `${SOURCE_TAG} ${displayName}`]
      );
    ...
    console.log('=== CP & SANITARY IMPORT SUMMARY ===');
    ...
    console.log(`package_items_mapping:         0  (no CP & Sanitary package matrix in xlsx)`);
  } catch (err) {
